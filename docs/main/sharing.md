...
For a fully self-contained sharing package: copy both `seamless.db` and a snapshot of the hashserver buffer directory. The buffer directory is already content-addressed: any file with checksum `abc123...` is at `buffers/ab/c1/23...` (or equivalent). You can rsync just the relevant files if you know which checksums you need.

`seamless.db` is also useful as a **reproducibility record**: it proves that your results were produced by transformations with known checksums. Attach it to a paper or dataset and anyone with Seamless can verify or replay your computations.
...
**When to use scratch**: large binary intermediates (raw simulation output, unfiltered alignments, etc.) that are cheap to recompute relative to their storage cost. The result checksum is real, durable, and participates in downstream caching — only the bytes are transient.
...
**Fingertipping** is the process of resolving a checksum when its buffer is not in the hashserver: look up which transformation produced that checksum (using the database's reverse lookup), and re-execute it to recover the bytes.

Fingertipping is the mechanism that makes scratch and sharing-by-checksum work. It "closes the loop" between identity and materialization.
...
This is the "give someone a checksum and they re-run it" sharing path. If you publish a transformation checksum alongside your `seamless.db`, anyone with Seamless can re-execute your exact computation — with the exact same inputs, code, and metadata that you used — and verify that they get the same result checksum.

The transformation dict (inputs, code, language, metadata) is identified by the checksum. As long as the transformation dict's buffer is available (in the hashserver or resolvable via fingertipping), `seamless-run-transformation` can execute it.
