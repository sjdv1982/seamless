...
- **standalone:** every write through a projection child raises `TypeError("Standalone projection writes are not supported")` (`CellBase._check_write_authority`), `p += 1` and `c += 1` raise `TypeError("Augmented Cell updates require a bound Cell")`, `c["b"] = v` raises `TypeError("Standalone Cell item assignment is not supported")`, and `c.b = v` raises `AttributeError` (*Implementation status*).
...
**Sub-path writes and augmented assignment work standalone too**, as the same read-modify-set without the controller: `cell.b = v`, `cell["b"] = v`, `cell += 1`, and every write through a projection child (`cell.b.set(v)`, `cell.b.value = v`, `cell.b.checksum = cs`, `cell.b.set_checksum(cs)`, `cell.b.buffer = buf`, `cell.b.set_buffer(buf)`, `p = cell.b; p += 1`) update `cell`'s root value (steps 2–4 above, synchronously; there is no concurrent writer, so no retry). Authority is checked on the root of the chain, as bound (*Authority*); the materializability rule above applies unchanged. *Contract ahead of code:* standalone, all of these raise today (*Writes through a handle*; *Implementation status*).
