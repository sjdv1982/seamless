...
The two modes share one class, one read API and one write API. They differ in exactly three places: who owns the state, whether a read waits or evaluates, and which operations exist at all (mounts, `prune()` and `block_reason` are bound-only). Those differences are marked throughout. A handle to an anonymous bound cell — a bound projection or `as_celltype` — is bound in ownership and unbound in how it reads (*Reads*, *Anonymous and projection handles*).
...
**A write through a projection handle is a pathed write to the parent, never a write to the handle's own node** — bound and standalone alike. With `p = ctx.a.b`, all six writes — `p.set(v)`, `p.value = v`, `p.set_buffer(buf)`, `p.buffer = buf`, `p.set_checksum(cs)`, `p.checksum = cs` — and augmented assignment (`p += 1`) perform the same transaction as `ctx.a.b = v`: an atomic read-modify-set of `a`'s root value (*Projections*), checked by the parent's authority rule above and never detaching. Retained and non-retained handles are the same thing: `ctx.a.b.set(10)` and `p.set(10)` are one operation, as are `ctx.a.b += 1` and `p += 1`.
...
**Standalone, the same rule holds.** On a standalone Cell `c` with `p = c.b`, the six writes and `p += 1` are a read-modify-set of `c`'s root value, performed synchronously in the caller's thread: the checksum and buffer forms are resolved at `p`'s `celltype`, the clearing forms raise `ValueError`, a write through a standalone `as_celltype` child raises `AuthorityError`, and the authority check applies to the root of the chain (*Authority*). A projection of a projection writes through each parent in turn, down to that root. The parent-side spellings are the same write: `c.b = v` and `c["b"] = v` are pathed writes to `c`, `del c["b"]` removes the key `b` from `c`'s root value (a standalone Cell has no edges, so there is none to remove with it), and `c += 1` is a read-modify-set of `c`'s own root value (*Projections*). Each is checked on the root of the chain like any other write. There is no controller and so no concurrent writer to serialize against; the write is visible to the next read of `c` and of any handle over it.

*Contract ahead of code:*

- **bound:** the four checksum and buffer forms fail at a sub-path today (*Implementation status*); `p.checksum = None` and its siblings are not refused but fail the same way; and a bound `as_celltype` returns a standalone snapshot rather than a handle (*Connecting*), so the `as_celltype`-handle refusal has nothing to apply to yet.
- **standalone:** every write through a projection child raises `TypeError("Standalone projection writes are not supported")` (`CellBase._check_write_authority`), `p += 1` and `c += 1` raise `TypeError("Augmented Cell updates require a bound Cell")`, `c["b"] = v` raises `TypeError("Standalone Cell item assignment is not supported")`, `del c["b"]` raises `TypeError("Standalone Cell item deletion is not supported")`, and `c.b = v` raises `AttributeError` (*Implementation status*).
...
**Sub-path writes and augmented assignment work standalone too**, as the same read-modify-set without the controller: `cell.b = v`, `cell["b"] = v`, `del cell["b"]`, `cell += 1`, and every write through a projection child (`cell.b.set(v)`, `cell.b.value = v`, `cell.b.checksum = cs`, `cell.b.set_checksum(cs)`, `cell.b.buffer = buf`, `cell.b.set_buffer(buf)`, `p = cell.b; p += 1`) update `cell`'s root value (steps 2–4 above, synchronously; there is no concurrent writer, so no retry). Authority is checked on the root of the chain, as bound (*Authority*); the materializability rule above applies unchanged. *Contract ahead of code:* standalone, all of these raise today (*Writes through a handle*; *Implementation status*).
...
- **Standalone sub-path writes and augmented assignment are refused.** The contract is that they are pathed writes to the standalone parent (*Writes through a handle*; *Projections*). Today every write verb on a standalone projection child raises `TypeError("Standalone projection writes are not supported")` (`CellBase._check_write_authority` refuses any Cell with a path); `p += 1` and `cell += 1` raise `TypeError("Augmented Cell updates require a bound Cell")` (`Cell._augmented`); `cell["b"] = v` raises `TypeError("Standalone Cell item assignment is not supported")`; `del cell["b"]` raises `TypeError("Standalone Cell item deletion is not supported")`; and `cell.b = v` raises `AttributeError`. *Verified against code (2026-09-25).* No write is silently lost.
