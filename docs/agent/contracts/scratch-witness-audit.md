...
- **Reading a handle.** `Cell.buffer`, `Cell.value` and `Pin.value` resolve and never fingertip; only an explicit `compute()` / `Transformation.run()` does work (`contracts/cells.md`).
