...
The declared pin celltype is preserved in concrete transformation tuples and
across schema edits, snapshots, and graph round-trips. `schema_celltypes` exposes
the independently derived schema types. The public errors are exported from
`seamless_transformer`.

`seamless_transformer.compiled_validation` owns the declaration whitelist and
Stage 2 checks. Construction checks literals as serialized values and checks
checksum/locally available HashType facts for deferred inputs, without fetching
input buffers. The executor validates every input before native compilation,
including replayed transformation dictionaries. Workflow Stage 1 failures block
before pin resolution.

The conservative baseline is scoped to the compiled **pin**, not its upstream
workflow data. Ordinary Expression conversions still run before that boundary.
Character buffers use byte lengths without terminators; empty buffers receive a
non-NULL pointer. The core serializer supports the non-finite, Boolean and complex
representations specified above.
