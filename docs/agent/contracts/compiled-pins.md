...
**Rule 4's null rejection is not enforced by today's general pin-null check.** Every compiled input pin is recorded internally as celltype `mixed`, whatever its declared celltype (`compiled_transformer.py`: `self._celltypes = {parameter.name: "mixed" for parameter in sig.inputs}`; `pretransformation.py`: `pretransformation_dict[pinname] = ("mixed", None, value)`). `transformation_utils.validate_pin_null` — the required-pin null check shared with `contracts/pins.md` — exempts celltypes `plain`, `mixed` and `bytes` from its `TypeError`, so a null value on a required compiled pin whose *declared* schema celltype is none of those (e.g. `float`) passes that check instead of being rejected by it. Rule 4 still applies, but no compiled-specific Stage 2 check currently enforces it: the null reaches native argument marshalling as Python `None`, and the failure that results is untraced and unclear rather than the intended pin-level error naming the pin, the declared celltype and the schema dtype/shape. This is the same gap `contracts/pins.md` already flags in general ("today's code does not yet honour a compiled pin's declared celltype"); fixing it is the celltype-recording change discussed in §10's Checksum consequences, which already rules that the resulting checksum change is correct. No test currently sends null into a compiled input pin (`seamless-workflow/tests/correctness/test_correctness_compiled.py` covers only a null *result*).

**The gap is wider than the null bypass: §3c's whitelist, §4's reporting, and §9's error classes are not implemented at all**, not merely bypassed for null:

- **The declared-pin-celltype whitelist (§3c) is not checked against the schema.** `CompiledCelltypesWrapper.__setitem__` delegates a non-`result` key straight to the general-purpose `CelltypesWrapper` (`compiled_transformer.py:372-376`), which validates against the ordinary 13 celltypes plus `deepcell`/`deepfolder`/`folder`/`module` — `contracts/pins.md`'s table, not this page's whitelist. `tf.celltypes.x = "plain"` on a scalar `float64` parameter is accepted without error today. *Verified:* none of `CompiledPinCelltypeError`, `CompiledPinCelltypeWarning`, `CompiledMixedValueError` or `CompiledPinSchemaError` (§9) exist anywhere under `seamless-transformer` (checked by name, 2026-09-20).
- **Assigning `tf.schema` resets every input pin's declared celltype to `mixed` instead of preserving it.** `CompiledMixin.schema`'s setter replaces the whole map unconditionally — `self._celltypes = {parameter.name: "mixed" for parameter in sig.inputs}` (`compiled_transformer.py:458`) — on every schema assignment. §4 step 3 ("preserve declared celltypes for surviving pin names") is not implemented: a `tf.celltypes.x` declared before a later `tf.schema = ...` is discarded, not carried forward, and no incompatibility is ever reported because nothing is preserved to check.
- **A compiled pin marked optional is not rejected, and can be silently dropped — the one outcome rule 3 forbids.** `tf.optional_pins` is `TransformerCore.optional_pins`, shared by every transformer kind with no compiled-specific check, and `compiled_transformer_to_pretransformation`'s output flows into the same `transformation_from_pretransformation` → `normalize_optional_pins_for_construction` path a Python/bash transformer uses, with no branch that special-cases `__compiled__`. A compiled pin declared optional and resolving to canonical null is therefore dropped from the transformation dict exactly as a Python pin's would be, rather than being rejected when declared or on graph import.

Together these mean nearly all of §3–§4 and §9 describe **prospective** contract from `compiled-transformer-celltypes-design-plan.md`, not current behaviour; only the schema-derived call signature (§3b's shapes/dtypes, feeding native argument marshalling directly) and the numpy-level scalar/array validation (`_validate_native_numpy_value`) exist today, organized around dtype and shape rather than around a declared pin celltype. An agent must not infer from a passing `tf.celltypes.x = ct` assignment, or from a compiled transformer accepting `tf.optional_pins`, that either is contractually valid.
