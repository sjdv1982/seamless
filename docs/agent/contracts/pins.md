...
This holds for **every route into a pin** — a pre-bound standalone pin, a call-time argument `tf(x=…)`, and a bound edge — and **reactive and snapshot runs agree**, including on the resulting checksum identity: `ctx.tf.run()` and `ctx.tf().run()` produce the same transformation checksum for the same inputs. (*Verified:* `_convert_pin_arguments` wraps a typed argument whose celltype differs in `Expression(arg, input_celltype=arg.celltype, celltype=<pin celltype>)`; `runtime_api._snapshot_transformer` builds the same converting Expression for a bound edge and for a stored producer; `Reactive._derive_transformer` converts through the Context's projection path.)
