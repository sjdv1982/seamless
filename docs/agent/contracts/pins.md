...
- **`Pin.fingertip()` does not exist.** The only entry point today is `Checksum.fingertip()`; the same item in `contracts/cells.md` covers both sides.
