...
> **One ruled vocabulary, across every page (2026-09-21).** *Publishing* is the buffer write; *recording* is what happens to a result checksum. This page previously used "publication" for the second act; it no longer does, and the pages that already used "publishing" for the buffer write (`contracts/scratch-witness-audit.md`, `contracts/expressions.md`, `contracts/identity-and-caching.md`) are the ones in agreement. The two acts have different rules: recording always happens on success, writing the buffer only when somebody increfs it non-scratch. The code's `_publish_expression_result` is named for neither — its docstring is "Publish bounded cache interest" and it registers a tempref — and is to be renamed **`_tempref_expression_result`**.
...
**Scratch.** Scratch acquisition does not register a buffer remotely, and later non-scratch interest upgrades registration monotonically. A **fingertip chain never publishes at any site** — client, jobserver or Dask worker alike — and the one exception is a fingertip performed on a *remote requester's* behalf, which writes the end result of the chain and nothing else (`contracts/expressions.md`, *Fingertipping is exempt from "where the data is"*). **Explicit public interest in a scratch result refholds and protects it**; an unrequested scratch result stays purgeable. See `contracts/scratch-witness-audit.md` for what scratch means.
...
- **A transfer write is not an ownership claim, and it is the one case that looks like an exception.** When the requester is a bare `Expression` asking for a *value*, the dispatch carries `scratch=False` although nobody will hold the result: the hashserver is the only channel by which the bytes can reach the requester (`contracts/expressions.md`, *The requester's scratch decision, and what a bare Expression carries*). The executing side takes a **non-scratch tempref** — bounded, as on the transformation path — so the rule above is intact at the mechanism level (a non-scratch incref is still what writes), and no durable claim is created: with nothing increfing it, the entry is as evictable as any other unheld buffer. Asking for a *checksum* carries `scratch=True` and writes nothing at all.

*Contract ahead of code:* Expression evaluation currently temprefs its result buffer non-scratch on every terminal branch, which queues a hashserver write regardless of who holds the result (`contracts/expressions.md`, *Status: publication and fingertipping*). The transformation branch of a fingertip does not.
