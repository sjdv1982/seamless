...
Examples:
  ...
  "--poi" => program parameters output input
    ...
    => tar --create --gzip --file /tmp/backup.tgz /home/user
       #no --verbose since it was specified as False; no specification gives an error
...
p: same as above, but no - is added, meaning that $parname must start with at least one - (validated)
