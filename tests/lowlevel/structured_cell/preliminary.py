...
def structured_transformer():
    ...
    c.input = StructuredCell(
        ...
        inchannels=[()],
        outchannels=[tuple(k) for k in tf_params.keys()],
    )
    c.result = cell("mixed")
    c.tf.connect(c.result)
...
with macro_mode_on():
    ...
    ctx.stf1 = structured_transformer()
    ctx.stf2 = structured_transformer()
    ctx.stf3 = structured_transformer()
    ctx.stf4 = structured_transformer()
    ...
    ctx.params_struc.outchannel_tf1.connect(ctf.stf1.input.inchannels[()])
