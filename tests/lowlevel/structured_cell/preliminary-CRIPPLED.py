...
with macro_mode_on():
    ...
    ctx.add_result = cell("int").set(0)
    ###ctx.add.result.cell().connect(ctx.params.inchannels[tf3 + ("offset",)])
    ###ctx.add.result.cell().connect(ctx.params.inchannels[tf4 + ("offset",)])7
    def func(checksum):
        if checksum is None:
            return
        value = ctx._get_manager().resolve(bytes.fromhex(checksum))
        ###ctx.add_result.set(value) # over-eager cancellation => will reset everything....
    ctx.add.result.cell()._set_observer(func)
    ctx.add_result.connect(ctx.params.inchannels[tf3 + ("offset",)])
    ctx.add_result.connect(ctx.params.inchannels[tf4 + ("offset",)])
