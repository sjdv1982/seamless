...
- Macro sealing / connection layers / connections to macro-generated contexts
  ...
  - Connections may only be added between the children of the sealed context and children of the *current context*
    The current context is the inner context whose macro is being run, or else the top-level context.
    The current context must be an ancestor of the sealed context (more precisely: be an ancestor of the context that is linked to the seal)
  - When a parent context is sealed, so will all child contexts. If they already were, they will be re-sealed using the parent seal.
  ...
  - Connections to such contexts are by default added to the *static connection layer* of the *current context*. A static connection layer consists of connections that are pairs of path names.
