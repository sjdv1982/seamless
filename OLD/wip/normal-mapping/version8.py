...
#edit(ctx.inscription)
ctx.inscription.connect(ctx.gen_texture.inscription)
ctx.inscription.connect(ctx.gen_normal_map.inscription)
ctx.inscription.set("Love you Isaure")

ctx.texture_filename = cell(str)
ctx.texture_filename.connect(ctx.gen_texture.filename)
ctx.normal_texture_filename = cell(str)
ctx.normal_texture_filename.connect(ctx.gen_normal_map.normal_texture_filename)

ctx.texture.connect(ctx.gen_normal_map.texture)
...
ctx.normal_map.connect(ctx.tri_program.array_s_normal_map)
ctx.normal_map.connect(ctx.ftri_program.array_s_normal_map)
...
ctx.triangle_tangents = cell("array").set_store("GL")
tf = ctx.gen_triangle_tangents = transformer({
    "indices": {"pin": "input", "dtype": "array"},
    "tangents": {"pin": "input", "dtype": "array"},
    "triangle_tangents": {"pin": "output", "dtype": "array"},
})
ctx.triangle_indices.connect(tf.indices)
ctx.tangents.connect(tf.tangents)
tf.triangle_tangents.connect(ctx.triangle_tangents)
tf.code.set("return tangents[indices]")
...
ctx.tangents.connect(ctx.tri_program.array_tangents)
ctx.triangle_tangents.connect(ctx.ftri_program.array_tangents)


#ctx.texture_filename.set("textures/Bricks.png")
#ctx.normal_texture_filename.set("textures/Bricks_NRM.png")
ctx.texture_filename.set("textures/Rock.png")
ctx.normal_texture_filename.set("")
#ctx.normal_texture_filename.set("textures/Rock_NRM.png")

ctx.amplitude = cell("array").set_store("GLTex", 2)
ctx.amplitude.connect(ctx.tri_program.array_s_amplitude)
ctx.amplitude.connect(ctx.ftri_program.array_s_amplitude)
ctx.gen_amplitude = transformer({"result": {"pin": "output", "dtype": "array"}})
ctx.gen_amplitude.result.connect(ctx.amplitude)
link(ctx.gen_amplitude.code.cell(), ".", "cell-gen-amplitude.py")

ctx.gen_uniforms.period.cell().set(1)
