...
class ExampleTransformer(Controller):
    ...
    def receive_update(self, inputpin, value):
        self.transformer.queue.append((inputpin, value))
        ...
        while 1:
            try:
                ...
                if self.output_finish.is_set():
                    if not len(self.output_queue):
                        break
                outputvalue = self.output_queue.popleft()
                self.output.update(outputvalue)
            except:
