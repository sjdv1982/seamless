    ...
    if hash_pattern in ("#", "##"):
        ...
        if not single_key:
            ...
            if has_star:
                ...
                sub_deep_structure = deep_structure.get(attribute)
                sub_hash_pattern = hash_pattern["*"]
                return access_deep_structure(
                    sub_deep_structure, sub_hash_pattern, path[1:]
                )
            elif key == "!":
    ...
    if has_star or not single_key:
        for key in deep_structure:
            ...
            _deep_structure_to_checksums(
                deep_structure[key], sub_hash_pattern, checksums, with_raw
            )
    else:
        for key in hash_pattern:
            if key.startswith("!"):
                ...
                for sub_deep_structure in deep_structure:
                    _deep_structure_to_checksums(
                        sub_deep_structure, sub_hash_pattern, checksums, with_raw
                    )
            else:
                assert list(deep_structure.keys()) == [key]
                _deep_structure_to_checksums(
                    deep_structure[key], hash_pattern[key], checksums, with_raw
                )
    ...
    if has_star or not single_key:
        ...
        for key in deep_structure:
            ...
            sub_result = _deep_structure_to_value(
                deep_structure[key], sub_hash_pattern, value_dict, copy=copy
            )
        ...
        for key in hash_pattern:
            if key.startswith("!"):
                ...
                for sub_deep_structure in deep_structure:
                    sub_result = _deep_structure_to_value(
                        sub_deep_structure, sub_hash_pattern, value_dict, copy=copy
                    )
...
async def deep_structure_to_value(deep_structure, hash_pattern, buffer_dict, copy):
    ...
    """
    checksums0 = deep_structure_to_checksums(
        deep_structure, hash_pattern, with_raw=True
    )
    futures = {}
    checksums = set()
...
def deep_structure_to_value_sync(deep_structure, hash_pattern, buffer_dict, copy):
    ...
    if not asyncio.get_event_loop().is_running():
        coro = deep_structure_to_value(deep_structure, hash_pattern, buffer_dict, copy)
    ...
    if has_star or not single_key:
        ...
        for key in d:
            if key in hash_pattern:
                ...
                sub_hash_pattern = hash_pattern["*"]
            sub_result = _build_deep_structure(sub_hash_pattern, d[key], c)
            result[key] = sub_result
    else:
        for key in hash_pattern:
            if key.startswith("!"):
                ...
                sub_hash_pattern = hash_pattern[key]
                for sub_d in d:
                    sub_result = _build_deep_structure(sub_hash_pattern, sub_d, c)
                    result.append(sub_result)
            else:
        ...
        for key in hash_pattern:
            if key.startswith("!"):
                ...
                for sub_value in value:
                    sub_result = _value_to_objects(sub_value, sub_hash_pattern, objects)
                    result.append(sub_result)
                return result
            else:
                assert list(value.keys()) == [key]
                sub_result = _value_to_objects(value[key], hash_pattern[key], objects)
                return {key: sub_result}


async def value_to_deep_structure(
    value, hash_pattern, *, cache_buffers=True, sync_remote_buffer_info=False
):
    """build deep structure from value"""
    try:
        objects = {}
        deep_structure0 = _value_to_objects(value, hash_pattern, objects)
    ...
    async def conv_obj_id_to_checksum(obj_id, raw):
        ...
        buffer_cache.guarantee_buffer_info(
            obj_checksum, "mixed", sync_to_remote=sync_remote_buffer_info
        )
...
def value_to_deep_structure_sync(
    value, hash_pattern, *, cache_buffers=True, sync_remote_buffer_info=False
):
    """This function can be executed if the asyncio event loop is already running"""
...
def write_deep_structure(checksum: Checksum, deep_structure, hash_pattern, path):
    ...
    result = write_deep_structure(
        checksum, sub_deep_structure, sub_hash_pattern, path[1:]
    )
    if result[0] == 2:
...
def apply_hash_pattern_sync(checksum, hash_pattern):
    ...
    value = deserialize_sync(buffer, checksum, "mixed", False)
