...
async def _prepare(macro, manager, max_running_tasks):
    ...
    for pinname, accessor in sorted(upstreams.items(), key=lambda item: item[0]):
        ...
        expression_checksum = await evaluate_expression(
            accessor.expression, manager=manager
        )
        ...
        buffer = Buffer(buffer, checksum=expression_checksum)
        value = await buffer.deserialize_async(celltype)
        if value is None:
...
class MacroManager:
    ...
    def _update_next_macro(self):
        ...
        if manager is None or manager._destroyed:
            return
        if (
            len(manager.taskmanager.tasks)
            + len(manager.taskmanager.upon_connection_tasks)
        ...
        def build_macrokeys(macros, parent_macro=0):
            ...
            macrokeys0 = [
                ((m[0].path if m[0] is not None else ()), m[0], m[1], m[2])
                for m in macrokeys00
            ]
            macrokeys = [
                (-len(m[0]), m[0], -len(m[2]), m[2], m[1], m[3]) for m in macrokeys0
            ]
            return macrokeys
        ...
        sibling_macros = [
            ...
            if m is macro
            or (
                m._gen_context is None
                and not m._void
                and m._get_macro() is parent_macro
            )
        ]
        if len(sibling_macros) == 1:
            proceed = True
        else:
            sibling_macrokeys = build_macrokeys(
                sibling_macros, parent_macro=parent_macro
            )
    ...
    def update_macro(self, macro):
        ...
        if macro in self.preparing_tasks:
            return
        self.preparing_tasks[macro] = asyncio.ensure_future(self._prepare_macro(macro))
