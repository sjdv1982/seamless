...
class Elision:
    def __init__(self, livegraph, macro, input_cells, output_cells):
        ...
        for cells, cells2 in [
            (input_cells, input_cells2),
            (output_cells, output_cells2),
        ]:
    ...
    def get_elision_checksum(self) -> Checksum:
        ...
        elision_buffer = Buffer(elision_dict, "plain")
        elision_checksum = elision_buffer.get_checksum()
    ...
    def destroy(self):
        ...
        for c in self.input_cells:
            if (
                c in livegraph.cell_to_macro_elision
    ...
    if cache_hit is None:
        ...
        if db_cache_hit is None:
            ...
            if elision_result_buffer is not None:
                # print("DB CACHE HIT!", macro, elision_checksum.hex())
                elision_result_buffer = Buffer(
                    elision_result_buffer, checksum=db_cache_hit
                )
                cache_hit = elision_result_buffer.deserialize("plain")
