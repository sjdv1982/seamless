...
def bridge_r(*, bridge_parameters, **kwargs):
    ...
    class RInterpreterError(ri.embedded.RRuntimeError):
        ...
        msg_prefix_template = (
            "Failed to parse and evaluate line %r.\n" "R error message: %r"
        )
        rstdout_prefix = "\nR stdout:\n"
    ...
    try:
        ...
        if rpacks.isinstalled("Cairo"):
            msg = (
                "An error occurred when trying to load the "
                + "R package Cairo'\n%s" % str(rre)
            )
        else:
            msg = textwrap.dedent(
        ...
        raise RInterpreterError(msg, None, None) from rre
    ...
    def setup_graphics(args):
        ...
        if device in ["png", "svg"]:
            ...
            if device == "png":
                ...
                grdevices.png("%s/Rplots%%03d.png" % graph_dir_fix_slashes, **argdict)
            elif device == "svg":
                argdict.pop("width")  # BUG in Cairo or rpy2?
                argdict.pop("height")  # BUG in Cairo or rpy2?
                cairo.CairoSVG("%s/Rplot.svg" % graph_dir_fix_slashes, **argdict)
        else:
        ...
        with contextlib.ExitStack() as stack:
            ...
            try:
                ...
                raise RInterpreterError(
                    code, str(exception), warning_or_other_msg
                ) from exception
    ...
    # Transfer input
    with localconverter(converter) as _cv:
        ro.r.assign("BLAH", [1, 2, 3])
        for pin in PINS:  # pylint: disable=undefined-variable
            if pin in ("code", "bridge_parameters"):
                continue
            print("INP", pin)
            try:
                ro.r.assign(pin, PINS[pin])  # pylint: disable=undefined-variable
            except NotImplementedError as exc:
                raise NotImplementedError(
                    "Pin '{}':".format(pin) + str(exc.args)
                ) from None
    ...
    try:
        ...
        # Run code
        with localconverter(converter) as _cv:
            code = PINS["code"]  # pylint: disable=undefined-variable
            evalR(code)
            if device in ["png", "svg"]:
                ro.r("dev.off()")
            device_off = True
