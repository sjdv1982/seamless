...
def get_bash_checksums():
    ...
    if _bash_checksums is None:
        ...
        semantic_code_checksum = _get_semantic(
            executor_code, Checksum(executor_code_checksum).bytes()
        )
        _bash_checksums = {}
...
def unbashify_docker(
    transformation_dict, semantic_cache, env: dict, execution_metadata: dict
):
    from seamless.workflow.core.direct.run import (
        prepare_code,
        prepare_transformation_pin_value,
    )
...
def unbashify(transformation_dict: dict, semantic_cache, execution_metadata: dict):
    ...
    from seamless.util.environment import (
    ...
    if env is not None and env.get("docker") is not None:
        ...
        if not (ok1 or ok2):
            return unbashify_docker(
                transformation_dict, semantic_cache, env, execution_metadata
            )
    ...
    conda_environment_checksum = prepare_transformation_pin_value(
        conda_environment_, "str"
    ).hex()
    ...
    semkey = (semantic_code_checksum.bytes(), "python", "transformer")
    semantic_cache[semkey] = [
        Checksum(bash_checksums["executor_code_checksum"]).bytes()
    ]
