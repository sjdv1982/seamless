"""Generate a dict-of-binary-objects (.o / .obj)"""
...
def compile(
    ...
    *,
    ...
    all_headers = {}
    source_files = {}
    for _objectname, object_ in binary_objects.items():
        for headername, header in object_["headers"].items():
            if headername in all_headers:
                old_header = all_headers[headername]
                if old_header != header:
                    raise ValueError(
                        "Two different header versions for '%s'" % headername
                    )
            assert header["language"] in (
                "c",
                "cpp",
            )  # for now, only C/C++ headers; don't know file extensions otherwise
    ...
    try:
        try:
            ...
            if not build_dir_may_exist:
                print(
                    "WARNING: compiler build dir %s already exists... this could be trouble!"
                    % build_dir
                )
        os.chdir(build_dir)
        ...
        for obj_file, object_ in binary_objects.items():
            ...
            for opt in object_["options"]:
                cmd.append(opt)
            cmd += [object_["output_flag"], os.path.abspath(obj_file)]
            ...
            if compiler_verbose:
                stderr += cmd2 + "\n"
            process = subprocess.run(cmd2, shell=True, capture_output=True, check=False)
...
def complete(module_definition, compilers, languages):
    """Complete a module definition using compilers and languages settings."""
    ...
    for objectname, object_ in module_definition["objects"].items():
        if "code" not in object_:
            raise ValueError("Binary Module, object '%s': no code defined" % objectname)

        o = m["objects"][objectname]
        if not "language" in object_:
            raise ValueError(
                "Binary Module, object '%s': no language defined" % objectname
            )
        ...
        if compiler_name is None:
            raise ValueError(
                "No compiler defined for compiled language '{}'".format(lang)
            )
        ...
        if target in ("release", "profile"):
            ...
            if target == "profile":
                profile_options = (
                    ...
                    if isinstance(profile_options, str)
