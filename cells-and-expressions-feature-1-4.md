...
9. **A deep dummy read is unvalidated, and `.buffer` contradicts `.value`.** On a `Cell("deepcell")` /
   ...
   **skips validation altogether**, and `.buffer` on the same Cell **raises** `HashTypeValidationError`,
   ...
   by hand is currently also the **only** way to reach a deep child's checksum. One further lag, minor
   and in the same area: the value rule presents members as `Checksum` objects, and the code hands back
   raw hex strings.
